...
const { coalesceFunks, call, runFunks } = require('../src/index.js')
...
test('`runFunks `', t => {
  ...
  const reducer = coalesceFunks((state = new Map(), action) => {
    switch (action.type) {
      case 'FOO':
        call(action, [mock, [action.payload]])
        return state
      case 'BAR':
        return state.set('funksHaveRun', true)
    }
  })
  ...
  t.deepEqual(calledWith, action.payload, 'calls funks')

  // wait to next tick of event loop for promise to resolve
  setTimeout(() => {
    t.deepEqual(getState().get('funksHaveRun'), true, 'dispatches actions')
    t.end()
  })
})
